...
@section('content')
    <div class="mt-5">
        <h3 class="fs-3 fw-semibold mt-2">Edit team</h3>
    </div>
    <div class="mt-2 bg-white dark:bg-gray-800 rounded-lg shadow divide-y divide-gray-100 dark:divide-gray-700">
        <form action="{{ route('teams.update', $team->id) }}" method="POST" enctype="multipart/form-data" class="p-5">
            @csrf
            @method('PUT')
            @if ($errors->any())
            ...
            <div class="row g-3 align-items-center">
                    ...
                    <div class="col-md-8 col-sm-12 p-3">
                        <input type="text" name="name" id="name" value="{{$team->name}}" class="w-100 form-control form-input form-input-bordered">
                    </div>
            </div>
                    ...
                    <div class="mt-1 text-center" id="image-preview">
                        @if($team->logo)
                            <div id="preview-image" class="preview-image">
                                <img src="{{ $team->logoUrl }}" class="img-thumbnail" alt="{{ $team->logo }}">
                            </div>
                        @else
                        ...
                        <div class="text-sm-start">
                            <label for="logo" class="relative cursor-pointer bg-white rounded fw-semibold">
